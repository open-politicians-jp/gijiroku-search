...
# 3. origin/mainから新ブランチを作成
git checkout -b feature/your-feature-name

# ❌ 禁止: ローカルmainからの分岐
# git checkout -b feature/name  # これは禁止
```

#### 🔀 ブランチ戦略・命名規則
- **`feature/`**: 新機能開発（例: `feature/user-authentication`）
- **`fix/`**: バグ修正（例: `fix/login-error`）
- **`docs/`**: ドキュメント更新（例: `docs/api-documentation`）
- **`refactor/`**: リファクタリング（例: `refactor/component-structure`）
- **`hotfix/`**: 緊急修正（例: `hotfix/production-crash`）

#### ✅ 正しいワークフロー
1. **最新main取得**: `git checkout main && git pull origin main`
2. **ブランチ作成**: `git checkout -b feature/name`
3. **開発作業**: コード変更・テスト実行
4. **コミット**: `git add . && git commit -m "説明"`
5. **プッシュ**: `git push -u origin feature/name`
6. **PR作成**: `gh pr create` でプルリクエスト作成
...
- **レビュー**: 重要な変更は必ずレビューを経てマージ

#### 💡 ベストプラクティス
- **作業前確認**: 必ずmainの最新状態を確認
- **小さなPR**: 大きな変更は複数のPRに分割
- **テスト完了**: `npm run build && npm run lint` 成功確認
- **コンフリクト回避**: こまめなmainからのrebase
