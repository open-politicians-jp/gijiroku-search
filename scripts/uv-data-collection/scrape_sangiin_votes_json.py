...
def main(bill_name="所得税法等の一部を改正する法律案"):
    base_url = "https://www.sangiin.go.jp/japanese/joho1/kousei/gian/217/"
    initial_url = urljoin(base_url, "gian.htm")
    output_md_path = "/Users/hironeko/Work/private/new-jp-search/tmp2.md"
    output_json_path = "/Users/hironeko/Work/private/new-jp-search/tmp2.json"
    ...
    try:
        ...
        bill_link = soup.find('a', string=re.compile(re.escape(bill_name)))
        if not bill_link:
            print(f"目的の法案リンク \"{bill_name}\" が見つかりませんでした。")
            return
        ...
        with open(output_md_path, 'w', encoding='utf-8') as md_file:
            ...
            for party_section in party_sections:
                ...
                md_file.write(f"| {party_name} | {sansei_count} | {hantai_count} |\n")
            md_file.write("\n")

            # --- Individual voter results ---
            md_file.write("## 議員別投票結果（政党別）\n\n")
            
            all_voting_data = get_voting_results(voting_page_response.url)
